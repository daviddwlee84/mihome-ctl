...
Browses remotes/keys from the cached ``mi-ir.json`` (no cloud call). Sending a key
or controlling the A/C hits the cloud inside a worker thread, reusing the cached QR
session via :func:`connector_from_session` — it never pops a QR (run ``mihome-ctl ir``
...
class MihomeApp(App):
    ...
    def __init__(self, ir: dict | None = None, state: StateDir | None = None) -> None:
        super().__init__()
    ...
    def on_mount(self) -> None:
        ...
        table.add_columns("name", "model", "keys", "kind")
        self._reload()
        table.focus()
